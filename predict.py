...
class Predictor(BasePredictor):
    def setup(self) -> None:
        ...
        try:
            ...
            if self.device == "cuda":
                ...
                if gpu_memory < 8:
                    logger.warning("⚠️ Less than 8GB GPU memory detected. Consider using 2B model.")
    ...
    def predict(
        ...
        try:
            ...
            width, height, num_frames = self._validate_dimensions(width, height, num_frames)
            ...
            logger.info(f"📐 Dimensions: {width}x{height}, {num_frames} frames @ {frame_rate}fps")
            ...
            infer(
                ...
                conditioning_media_paths=conditioning_media_paths if conditioning_media_paths else None,
                conditioning_strengths=conditioning_strengths if conditioning_strengths else None,
                conditioning_start_frames=conditioning_start_frames if conditioning_start_frames else None,
                device=self.device
            )
            
            # Validate output file
            if not os.path.exists(output_path):
                raise RuntimeError("Video generation completed but output file was not created")
            
            file_size = os.path.getsize(output_path)
            if file_size == 0:
                raise RuntimeError("Generated video file is empty")
            
            logger.info(f"✅ Video generation completed: {output_path} ({file_size/1024/1024:.1f}MB)")
